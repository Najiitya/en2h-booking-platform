...
### 🏷️ Services
*Note: Service management endpoints require a valid JWT Bearer Token in the Authorization header.*
...
*   `GET /services/:id` - Retrieve a specific service by UUID (Public)
*   `PATCH /services/:id` - 🔒 Update service pricing or duration
*   `DELETE /services/:id` - 🔒 Remove a service

### 📅 Bookings
*Note: Customers can create bookings without authentication. A booking must reference a valid `serviceId`, and `appointmentDate` cannot be in the past.*
...
*   `PATCH /bookings/:id` - Update a booking's status (Note: Cancelled bookings cannot be marked as completed)
